...
(private val farmAgreementID: UniqueIdentifier) : FlowLogic<SignedTransaction>() {
    ...
    override fun call(): SignedTransaction {
        ...
        val setStatusCommand = Command(FarmAgreementContract.Commands.SetFarmAgreementStatusCancelled(), inputFarmAgreementState.participants.map { it.owningKey })
        val outputState = inputFarmAgreementState.copy(status = FarmAgreementStatus.CANCELLED)
...
@InitiatedBy(CancelAgreementFlow::class)
class CancelAgreementFlowResponder(val counterpartySession: FlowSession) : FlowLogic<SignedTransaction>() {
    @Suspendable
    override fun call():SignedTransaction {
